...
socket.addEventListener('message', function (data) {
	...
	if (jsonData.type == 'join') {
		...
		document.getElementById(
			'users'
		).innerHTML += `<li class="user_element" style="background-color: rgba(${jsonData.data.color['r']}, ${jsonData.data.color['g']}, ${jsonData.data.color['b']}, 0.1);">
		...
		document.getElementById('users').style.maxHeight = `${
			document.getElementById('users').childNodes[1].clientHeight * 4
		}px`
		document.getElementById('totalUsers').innerText =
			Number(document.getElementById('totalUsers').innerText) + 1
	...
	if (
		jsonData.type == 'join' ||
		jsonData.type == 'leave' ||
		(jsonData.type == 'message' && jsonData.data.oppo)
	) {
		const scrolldown = document.getElementById('scrolldown')
		const allMessages = document.querySelectorAll('.msg')
		if (
			Number(messageBox.scrollHeight - messageBox.clientHeight - messageBox.scrollTop) <=
			allMessages[allMessages.length - 1].clientHeight +
				allMessages[allMessages.length - 2].clientHeight
		) {
			document
				.getElementById('messages')
				.scrollTo(0, document.getElementById('messages').scrollHeight)
